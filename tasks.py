...
# Variables may be overwritten in invoke.yml or by the environment variables INVOKE_CONFIG_COMPLIANCE_xxx
...
def _is_compose_included(context, name):
    return f"docker-compose.{name}.yml" in context.config_compliance.compose_files
...
def docker_compose(context, command, **kwargs):
    ...
    build_env = {
        ...
        "COMPOSE_HTTP_TIMEOUT": context.config_compliance.compose_http_timeout,
        "NAUTOBOT_VER": context.config_compliance.nautobot_ver,
        "PYTHON_VER": context.config_compliance.python_ver,
        **kwargs.pop("env", {}),
    }
    compose_command_tokens = [
        "docker compose",
        f"--project-name {context.config_compliance.project_name}",
        f'--project-directory "{context.config_compliance.compose_dir}"',
    ]

    for compose_file in context.config_compliance.compose_files:
        compose_file_path = os.path.join(context.config_compliance.compose_dir, compose_file)
...
def run_command(context, command, **kwargs):
    ...
    if is_truthy(context.config_compliance.local):
...
def build(context, force_rm=False, cache=True):
    ...
    print(f"Building Nautobot with Python {context.config_compliance.python_ver}...")
...
def _get_docker_nautobot_version(context, nautobot_ver=None, python_ver=None):
    ...
    if python_ver is None:
        python_ver = context.config_compliance.python_ver
    dockerfile_path = os.path.join(context.config_compliance.compose_dir, "Dockerfile")
...
def lock(context, check=False, constrain_nautobot_ver=False, constrain_python_ver=False):
    ...
    if constrain_nautobot_ver:
        ...
        if constrain_python_ver:
            command += f" --python {context.config_compliance.python_ver}"
        try:
            ...
            command = f"poetry add --lock git+https://github.com/nautobot/nautobot.git#{context.config_compliance.nautobot_ver}"
            if constrain_python_ver:
                command += f" --python {context.config_compliance.python_ver}"
            run_command(context, command)
    else:
...
def makemigrations(context, name=""):
    ...
    command = "nautobot-server makemigrations config_compliance"
...
@task(
    help={
        "version": "Version of Config Compliance to generate the release notes for.",
    }
)
...
def pylint(context):
    ...
    command = 'pylint --init-hook "import nautobot; nautobot.setup()" --rcfile pyproject.toml config_compliance'
    run_command(context, command)
...
def unittest_coverage(context):
    ...
    command = "coverage report --skip-covered --include 'config_compliance/*' --omit *migrations*"
...
def tests(context, failfast=False, keepdb=False, lint_only=False):
    ...
    if not is_truthy(context.config_compliance.local):
